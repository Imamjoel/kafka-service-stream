...
public class FeedbackFiveStream {
    ...
    public KStream<String, FeedbackMessage> kstreamFeedback(StreamsBuilder builder) {
        ...
        feedbackStream[0].through("t.commodity.feedback-five-good-count").groupByKey().count().toStream().
                to("t.commodity.feedback-five-good-count");

        feedbackStream[0].through("t.commodity.feedback-five-bad-count").groupByKey().count().toStream()
